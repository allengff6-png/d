...
/**
 * Shortcode to render the prank overlay and script
 *
...
function wdp_prank_shortcode($atts) {
    ...
    // External images used (mirroring original Python script sources)
    ...
    ob_start();
    ?>
    <div class="wdp-container" data-total="<?php echo esc_attr($total); ?>" data-batch="<?php echo esc_attr($batch); ?>" data-delay="<?php echo esc_attr($delay); ?>" data-autostart="<?php echo esc_attr($autostart); ?>" data-bg="<?php echo esc_url($background_url); ?>" data-logo="<?php echo esc_url($logo_url); ?>" data-final="<?php echo esc_url($final_url); ?>">
    ...
    <script>
    (function(){
        const container = document.currentScript.previousElementSibling.previousElementSibling; // .wdp-container
        if (!container || !container.classList.contains('wdp-container')) return;
        ...
        function closeOverlay(){
            ...
            isRunning = false;
            // Remove all child popups
            document.querySelectorAll('.wdp-popup').forEach(function(node){ node.remove(); });
        }
        ...
        if (autostart) {
            ...
            // Wait a moment before starting (similar to root.after in Tkinter)
